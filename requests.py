...
def format_user_with_nationality(user):
    ...
    # Handle nationality and height
    nationality_code = user.get('nationalityCode', 'N/A')
    height_raw = user.get('height', 'N/A')
    if isinstance(height_raw, str) and "|" in height_raw:
        height_val, height_unit = height_raw.split("|", 1)
        height = f"{height_val} {height_unit}"
    else:
        height = height_raw if height_raw != 'N/A' else 'N/A'
    ...
    card = (
        ...
        f"<b>Nationality Code:</b> {nationality_code}\n"
        f"<b>Height:</b> {height}\n"
...
async def store_token_and_show_card(msg_obj, login_result, creds):
    ...
    if access_token:
        ...
        if user_data:
            ...
            # Add extra fields if present (height, nationalityCode, etc.)
